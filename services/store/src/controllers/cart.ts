...
export const addToCart = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      message: "Invalid store and item id",
    });
  }

  const cartFromDifferentStore = await Cart.findOne({
    userId,
    storeId: { $ne: storeId },
  });
  ...
  if (cartFromDifferentStore) {
    return res.status(400).json({
      message:
        "You can order from only one store at a time. Please clear your cart first to add items from this store.",
    });
  }

  const cartItem = await Cart.findOneAndUpdate(
    { userId, storeId, itemId },
    {
      $inc: { quauntity: 1 },
      $setOnInsert: { userId, storeId, itemId },
    },
